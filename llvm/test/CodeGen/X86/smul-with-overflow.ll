...
; X86-NEXT:    leal (%eax,%edx), %edi
; X86-NEXT:    movl %ebp, %eax
; X86-NEXT:    leal (%edi,%ebp), %edx
...
; X86-NEXT:    movzbl (%esp), %edx # 1-byte Folded Reload
...
; X86-NEXT:    movl %eax, {{[-0-9]+}}(%e{{[sb]}}p) # 4-byte Spill
; X86-NEXT:    movl {{[-0-9]+}}(%e{{[sb]}}p), %eax # 4-byte Reload
; X86-NEXT:    leal (%eax,%ebp), %eax
; X86-NEXT:    movl {{[-0-9]+}}(%e{{[sb]}}p), %ecx # 4-byte Reload
; X86-NEXT:    leal (%eax,%ecx), %ecx
...
; X64-NEXT:    movl %r15d, %r8d
; X64-NEXT:    andl $1, %r8d
; X64-NEXT:    movq %r8, %rdx
; X64-NEXT:    negq %rdx
; X64-NEXT:    movq %rdx, %rsi
; X64-NEXT:    xorq %rbp, %rsi
; X64-NEXT:    sarq $63, %rbp
; X64-NEXT:    xorq %rbp, %r11
; X64-NEXT:    xorq %rbp, %rbx
; X64-NEXT:    orq %rbx, %r11
; X64-NEXT:    xorq %rbp, %r9
