...
; CHECK:       # %bb.0:
; CHECK-NEXT:    vfnmsub213ss {{.*#+}} xmm1 = -(xmm0 * xmm1) - xmm2
; CHECK-NEXT:    vxorps {{\.?LCPI[0-9]+_[0-9]+}}(%rip), %xmm1, %xmm0
; CHECK-NEXT:    retq
  %negx = fneg float %x
  %negz = fneg float %z
...
; CHECK:       # %bb.0:
; CHECK-NEXT:    vfmadd213ss {{.*#+}} xmm1 = (xmm0 * xmm1) + xmm2
...
; CHECK:       # %bb.0:
; CHECK-NEXT:    vfnmsub213sd {{.*#+}} xmm1 = -(xmm0 * xmm1) - xmm2
; CHECK-NEXT:    vxorpd {{\.?LCPI[0-9]+_[0-9]+}}(%rip), %xmm1, %xmm0
; CHECK-NEXT:    retq
  %negx = fneg double %x
  %negz = fneg double %z
...
; CHECK:       # %bb.0:
; CHECK-NEXT:    vfmadd213sd {{.*#+}} xmm1 = (xmm0 * xmm1) + xmm2
